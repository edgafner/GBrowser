...
fun Finder.popupX(@Language("xpath") xpath: String? = null) = x(xpath ?: "//div[@class='HeavyWeightWindow']", PopupUiComponent::class.java)

fun Finder.popupMenuX(@Language("xpath") xpath: String? = null) = x(xpath ?: "//div[@class='MyMenu']", PopupMenuUiComponent::class.java)
...
class PopupMenuUiComponent(data: ComponentData) : UiComponent(data) {
  ...
  fun findMenuItemByTextX(text: String) =
    menuItems.list().firstOrNull { it.getText() == text } ?: throw AssertionError("No item with text '$text' found in popup '${this.searchContext}'")

  fun findMenuItemByContainingTextX(text: String) =
    menuItems.list().firstOrNull { it.getText().contains(text) } ?: throw AssertionError("No item with text '$text' found in popup '${this.searchContext}'")

  fun select(vararg items: String) {
    items.forEach { item ->
      waitForOne(message = "Find item: $item", timeout = 5.seconds, getter = { menuItems.list() }, checker = { it.getText() == item }).click()
    }
  }
...
class PopupItemUiComponent(data: ComponentData) : UiComponent(data) {
  ...
  fun getIconPath() = "path=(.*),".toRegex().find(popupComponent.getIcon().toString())?.let { it.groups.last()?.value ?: "empty" } ?: "empty"
