...
class GBrowserToolWindowDevToolsFactory : ToolWindowFactory, DumbAware {
  ...
  object Companion {
    private val LOG = logger<GBrowserToolWindowDevToolsFactory>()

    fun createTab(project: Project, browser: GCefBrowser, tabName: String = "New Tab") {
      LOG.debug("createTab called with project: $project, browser: $browser, tabName: $tabName")

      val toolWindowManager = ToolWindowManager.getInstance(project)
      val toolWindow = toolWindowManager.getToolWindow(GBrowserUtil.DEVTOOLS_TOOL_WINDOW_ID)

      if (toolWindow == null) {
        LOG.warn("DevTools tool window wasn't found with ID: ${GBrowserUtil.DEVTOOLS_TOOL_WINDOW_ID}")
        return
      }

      LOG.debug("DevTools tool window found: $toolWindow")
      ...
      if (existingContent == null) {
        LOG.debug("No existing content found, creating new content")
        createContent(toolWindow, browser, tabName)
      } else {
        LOG.debug("Existing content found, showing it")
